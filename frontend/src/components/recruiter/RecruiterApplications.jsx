...
const RecruiterApplications = ({ jobId, jobTitle, onBack }) => {
  ...
  const fetchApplications = async () => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || 'https://talentmatch-ai-intelligent-recruiting.onrender.com')}/recruiter/jobs/${jobId}/applications`, {
  ...
  const handleStatusChangeSubmit = async (e) => {
    ...
    try {
      ...
      if (newStatus === 'Interview') {
        res = await fetch(`${(import.meta.env.VITE_API_URL || 'https://talentmatch-ai-intelligent-recruiting.onrender.com')}/recruiter/applications/${selectedApp.id}/schedule-interview`, {
          method: 'POST',
          headers: {
        ...
        res = await fetch(`${(import.meta.env.VITE_API_URL || 'https://talentmatch-ai-intelligent-recruiting.onrender.com')}/recruiter/applications/${selectedApp.id}/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${user.token}`
  ...
  const handleViewProfile = async (app) => {
    ...
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || 'https://talentmatch-ai-intelligent-recruiting.onrender.com')}/recruiter/candidates/${app.candidate_id}/portfolio`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      if (res.ok) {
        const data = await res.json();
            ...
            <form onSubmit={handleStatusChangeSubmit} className="p-6 space-y-4">
              ...
              {newStatus === 'Interview' && (
                <div className="space-y-4 pt-2 border-t border-slate-200 dark:border-slate-700">
                  <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300">Interview Scheduling</h4>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Scheduled Date & Time (UTC)</label>
                    <input 
                      type="datetime-local" 
                      required 
                      value={scheduledTime}
                  ...
                  <p className="text-xs text-slate-500 flex items-center gap-1">
                    <Clock className="w-3 h-3" /> An email invitation with a Google Meet link will be sent.
                  </p>
                </div>
              )}
          ...
          <div className="bg-slate-50 dark:bg-slate-950 rounded-2xl w-full max-w-5xl h-full max-h-[90vh] shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900 shrink-0">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <User className="w-5 h-5" /> 
                {portfolioData?.profile ? `${portfolioData.profile.first_name} ${portfolioData.profile.last_name}'s Profile` : `Candidate #${viewingApp.candidate_id} Profile`}
              </h3>
